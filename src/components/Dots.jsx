...
export default function Dots({ total, current, onSelect }) {
  ...
  return (
    <div className="dots">
      {start > 0 && (
        <button className="dot-btn dot-edge" onClick={() => onSelect(0)} aria-label="Ir para imagem 1" />
      )}
      {items.map((i) => (
        <button
          key={i}
      ...
      {end < total - 1 && (
        <button className="dot-btn dot-edge" onClick={() => onSelect(total - 1)} aria-label={`Ir para imagem ${total}`} />
      )}
    </div>
  );
}
